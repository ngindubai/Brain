...
source: https://claude.ai (architecture sessions, 23 June 2026)
---

# Orwell Corp stack decisions

Durable record of the settled architecture for the Orwell-Corp repo (`ngindubai/Orwell-Corp`), the realised version of the [[areas/agent-org/agent-org]] engine. Supersedes earlier "Render" and "Node/BullMQ" assumptions in the trackers. This is the meta-record; the executable plan lives in the repo as the build-next plan and the dossier.
...
- The "self-learning" is the runtime writing and refining its own skills plus persistent memory, not a model retraining itself. See [[research/hermes-agent-personal-ai-os]].
...
Main and orchestrator steps on Claude; leaf and routine subagents on the Hermes 4 70B; only summaries return to the parent; caching on the briefs. This is the "Hermes runs the routine, Claude does the thinking" plan, which is the intended use of the runtime.
...
- [ ] S1 skills layer: retire the static clones, install a pinned audited manifest, curator opt-in (see [[research/agent-skills-ecosystem]])
