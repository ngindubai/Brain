...
Area: [[Products]]. Spare-time hobby. FiveM zombie extraction-survival server, QBox base, ZAP-Hosting.

## What we have learnt

- Seven-pass source review done. City Bible master HTML compiled.
- Fix pack: nl_deathdrop, nl_taxifix, v-containerrobbery config fix.
...
Done: Source review, City Bible, custom resources written and pushed to the repo.

Frontier: The flattened nl_deathdrop is in the repo but not yet uploaded via FTP, so the server still warns.

## Next actions

1. Stop the server, delete the existing nl_deathdrop folder on the FTP right pane.
2. Upload the flattened folder, verify only fxmanifest.lua and main.lua.
